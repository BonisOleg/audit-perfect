...
class GuardedUserAdmin(BaseUserAdmin, ModelAdmin):
    form = UserChangeForm
    ...
    def delete_queryset(self, request, queryset: QuerySet[User]):
        ...
        if allowed:
            super().delete_queryset(
                request, queryset.filter(pk__in=[user.pk for user in allowed])
            )


class GuardedGroupAdmin(BaseGroupAdmin, ModelAdmin):
    pass
